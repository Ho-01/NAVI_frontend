...
export default class ResultScene extends Phaser.Scene {
   ...
   */
  create(data){
    const { isCorrect=false, label="", wrongExplain="", prevKey, prevCfg, nextScene, nextParam } = data || {};
    ...
    // 1) 블랙 페이드
    const black = this.add.rectangle(w/2, h/2, w, h, 0xf00000, 0).setDepth(10);
    this.tweens.add({
      targets: black, alpha: 1, duration: 300, ease: EASE_FADE,
      onComplete: () => {
        // 1-1) 300ms 홀드 후 타이틀 카드
        this.time.delayedCall(300, () =>
          this.showTitleCard({ isCorrect, label, wrongExplain, prevKey, prevCfg, nextScene, nextParam })
        );
      }
    });
  }

  showTitleCard({ isCorrect, label, wrongExplain, prevKey, prevCfg, nextScene, nextParam }){
    const w = this.scale.width, h = this.scale.height;
